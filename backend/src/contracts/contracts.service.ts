...
export class ContractsService {
  ...
  async findAll(driverId?: string) {
    ...
    const stats = {
      ...
      completedThisMonth: contracts.filter(
        (c: any) => c.status === 'Completed' && new Date(c.updatedAt) >= startOfMonth,
      ).length,
    ...
    const formattedContracts = contracts.map((c: any) => ({
      ...
      unitMileage:
        c.checkoutMileage !== undefined ? c.checkoutMileage : c.unitId?.mileage || 0,
  ...
  async create(body: any, currentUser?: any) {
    ...
    if (contractType === 'Shop' && !body.clientId) {
      throw new BadRequestException('Client is required for Shop contracts.');
    }
  ...
  async update(id: string, body: any, currentUser?: any) {
    ...
    if (body.driverId !== undefined) updatedData.driverId = body.driverId || null;
    if (body.deliveryDriverId !== undefined) updatedData.deliveryDriverId = body.deliveryDriverId || null;
    if (body.returnDriverId !== undefined) updatedData.returnDriverId = body.returnDriverId || null;

    if (body.returnFuelLevel !== undefined) updatedData.returnFuelLevel = Number(body.returnFuelLevel);
    if (body.checkoutFuelLevel !== undefined) updatedData.checkoutFuelLevel = Number(body.checkoutFuelLevel);
    ...
    if (body.deliveryStatus === 'Delivered' && existingContract.deliveryStatus !== 'Delivered') {
      ...
      this.notifyAdmin({ contractId: existingContract._id.toString(), type: 'vehicle_delivered' }).catch(
    ...
    try {
      const count = await this.logModel.countDocuments();
      const moneyCollectedInfo =
        body.returnAmountCollected !== undefined
      ...
      await this.logModel.create({
        ...
        description: `Contract ${updatedContract._id.toString().substring(0, 8).toUpperCase()} was ${
          body.status === 'Completed' ? 'completed/returned' : 'updated'
        }.${moneyCollectedInfo}`,
        type: 'edit',
        ip: '127.0.0.1',
      });
      ...
      if (body.status === 'Completed' && body.status !== existingContract.status) {
        await this.notificationModel.create({
          ...
          message: `Contract ${updatedContract._id.toString().substring(0, 8).toUpperCase()} was returned. Driver confirmed collecting ${
            body.returnAmountCollected !== undefined ? `$${body.returnAmountCollected}` : 'remaining balance'
          } to admin.`,
          type: 'alert',
        });
  ...
  async patch(id: string, body: any, currentUser?: any) {
  ...
  async notifyDriver(payload: { driverId: string; contractId: string; type: string }) {
    ...
    if (driverPhone && instanceId && token) {
      try {
        ...
        if (response.ok) results.whatsapp = true;
      } catch (waErr) {
  ...
  async notifyAdmin(payload: { contractId: string; type: string }) {
  ...
  async sendEmail(payload: { email: string; clientName: string; pdfUrl: string; contractId?: string }) {
  ...
  async sendWhatsApp(payload: { phone: string; message: string; pdfUrl?: string; contractId?: string }) {
